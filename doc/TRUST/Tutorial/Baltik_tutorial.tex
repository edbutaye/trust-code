...
\title[TRUST Baltik Project Tutorial V1.9.8beta]{TRUST Baltik Project Tutorial V1.9.8beta}
...
\item Build an :
\begin{itemize}
\item semi-optimized -O3 option and assert enabled mode:\\
\texttt{\$ make semi\_optim}\\
\texttt{\$ ls \$exec\_semi\_optim}
...
{\color{red} Notice that TRUST optimized binary for profiling or a TRUST optimized binary for test coverage must exist in order to be able to compile your baltik's profiling or test coverage executable.} \\
\end{itemize}
...
%\begin{block}{}
%\textbf{NB:} "Interprete::interpreter()" method is called first,
...
\fbox{ \begin{minipage}[c]{0.9\textwidth}
\begin{alltt}
 my\_new\_keyword \{ {\bf{domain}} dom\_name {\bf{option}} 0 \}
\end{alltt}
\end{minipage}}
\end{center}
...
\fbox{ \begin{minipage}[c]{0.9\textwidth}
\begin{alltt}
 my\_new\_keyword \{ {\bf{problem}} pb\_name {\bf{option}} 1 \}
\end{alltt}
\end{minipage}}
\end{center}
\end{itemize}
...
\begin{frame}
\frametitle{Modify your cpp class (Part 1)}
\begin{block}{Example of Param use}
%\vspace{-0.8cm}
\begin{center}
...
\textcolor{orange}{param}.ajouter("problem",\&pb\_name,Param::REQUIRED); \\
...
\item As said before, we want to read this syntax from the data:
\begin{center}
\fbox{ \begin{minipage}[c]{0.9\textwidth}
\begin{alltt}
\small{
{\bf{my\_first\_class}} \{ {\bf{domain}} dom {\bf{option}} 0 \}
\textcolor{blue}{\# dom is domain name \#}
}
\end{alltt}
\end{minipage}}
\end{center}

\item here is how to do:
\begin{center}
\fbox{ \begin{minipage}[c]{0.9\textwidth}
\begin{alltt}
\small{
...
\{\\
{\bf{Nom}} domain\_name;\\
... \textcolor{blue}{// read params from the datafile}\\
\textcolor{blue}{// Retrieve the domain object name given by the user, and find}\\
\textcolor{blue}{// back the actual instance in memory:}\\
{\bf{Objet\_U\&}} obj = Interprete::objet(domain\_name);\\
\textcolor{blue}{// Sanity check: if read domain name does not correspond to a domain, exit}\\
{\bf{if}}(!sub\_type(Domaine, obj))\\
{\bf{Process::exit(}}"Object passed to 'Class' is of wrong type!"{\bf{);}}\\
\textcolor{blue}{// Fill the local reference to domain:}\\
Domaine\& domaine\_ref\_ = {\bf{ref\_cast}}(Domaine, obj);\\
...\\
\}
}
\end{alltt}
...
\fbox{ \begin{minipage}[c]{0.9\textwidth}
\begin{alltt}
{\bf{my\_first\_class}} \{ {\bf{domain}} dom {\bf{option}} 0 {\bf{problem}} pb \}
\end{alltt}
\end{minipage}}
\end{center}
...
\item Go the the directory where to create your notebook:\\
\texttt{\$ cd \$project\_directory}\\
